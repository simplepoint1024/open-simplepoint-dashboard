package org.simplepoint.security.authentication;
...
public class TotpVerifier {
  ...
  /** Returns the actual accepted counter so callers can persist replay protection. */
  public Long matchingStep(String secret, String code, Instant now) {
    ...
    for (long offset = -1; offset <= 1; offset++) {
      if (generateCode(secret, timeStep + offset) == codeInt) {
