...
public class AiWorkflowExecutionEngine {
  ...
  public void advance(final ExecutionTask task) {
    ...
    Optional<AiWorkflowNodeExecution> failed = nodes.values().stream()
        ...
        .findFirst();
    if (failFast && failed.isPresent()) {
      AiWorkflowNodeExecution node = failed.orElseThrow();
      failExecution(
          execution,
          plan,
          nodes,
  ...
  private void refreshWaiting(
    ...
    for (AiWorkflowNodeExecution node : nodes.values()) {
      if (node.getStatus() == WorkflowNodeExecutionStatus.WAITING) {
        refreshNormalWait(execution, plan, node, input, outputs, now);
  ...
  private void refreshNormalWait(
    ...
    switch (node.getNodeType()) {
      ...
      case "human" -> refreshHuman(execution, node, now);
      case "wait" -> {
  ...
  private void refreshAgent(
    ...
    if (child.getStatus() == AgentExecutionStatus.SUCCEEDED) {
      ...
      failNode(
          ...
          optional(child.getErrorMessage(), "Agent child execution failed"),
          now
      );
    }
  }
  ...
  private void refreshSkill(
    ...
    if (child.getStatus() == SkillExecutionStatus.SUCCEEDED) {
      ...
      failNode(
          ...
          optional(child.getErrorMessage(), "Skill child execution failed"),
          now
      );
    }
  }
  ...
  private void launchAgent(
    ...
    AiAgentExecution child =
        agentExecutionService.startVersionForWorkflow(
    ...
    node.setChildResourceId(dependency.resourceId());
    node.setChildResourceVersionId(dependency.resourceVersionId());
    node.setChildExecutionId(child.getId());
    node.setStatus(WorkflowNodeExecutionStatus.WAITING);
    nodeRepository.save(node);
    waitingEvent(execution, node, child.getId(), now);
  }
  ...
  private void launchSkill(
    ...
    AiSkillExecution child =
        skillExecutionService.startVersionForWorkflow(
  ...
  private void failNode(
    ...
    node.setErrorCode(optional(code, "WORKFLOW_NODE_FAILED"));
    node.setErrorMessage(bounded(message));
    node.setCompletedAt(now);
    nodeRepository.save(node);
    eventPublisher.publish(
        execution,
  ...
  private void failExecution(
    ...
    execution.setErrorCode(optional(code, "WORKFLOW_EXECUTION_FAILED"));
    execution.setErrorMessage(bounded(message));
  ...
  private void advanceCompensation(
    ...
    Optional<AiWorkflowNodeExecution> failed = nodes.values().stream()
        ...
        .findFirst();
    if (failed.isPresent()) {
      execution.setErrorCode("WORKFLOW_COMPENSATION_FAILED");
      execution.setErrorMessage(
          failed.orElseThrow().getErrorMessage()
      );
      finalizeFailure(execution, now);
      return;
    }
  ...
  private void refreshCompensation(
    ...
    if (child.getStatus() == SkillExecutionStatus.SUCCEEDED) {
      ...
      node.setErrorCode("WORKFLOW_COMPENSATION_CHILD_"
          + child.getStatus().name());
      node.setErrorMessage(optional(
