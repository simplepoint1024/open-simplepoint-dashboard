...
public interface JpaAiSkillExecutionRepository
  ...
  @Query("""
      ...
      where execution.skillId = :skillId
        and execution.sourceType =
          org.simplepoint.plugin.ai.skill.api.model.SkillExecutionSource.PUBLISHED
  ...
  @Query("""
      select execution from AiSkillExecution execution
      where execution.skillId = :skillId
        and execution.draftId = :draftId
        and execution.draftRevision = :draftRevision
        and execution.sourceType =
          org.simplepoint.plugin.ai.skill.api.model.SkillExecutionSource.DRAFT
        and execution.scopeType = :scopeType
        and ((:tenantId is null and execution.tenantId is null)
          or execution.tenantId = :tenantId)
        and execution.idempotencyKeyHash = :idempotencyKeyHash
  ...
  @Query("""
      ...
      where execution.skillId = :skillId
        ...
        and ((:tenantId is null and execution.tenantId is null)
          or execution.tenantId = :tenantId)
        and execution.sourceType = :sourceType
        and execution.deletedAt is null
      order by execution.createdAt desc
      """)
  Page<AiSkillExecution> findAllActiveBySkillScopeAndSource(
      ...
      @Param("scopeType") AiResourceScope scopeType,
      @Param("tenantId") String tenantId,
      @Param("sourceType") SkillExecutionSource sourceType,
      Pageable pageable
  );
  ...
  @Query("""
      ...
      where execution.skillId = :skillId
        ...
        and ((:tenantId is null and execution.tenantId is null)
          or execution.tenantId = :tenantId)
        and execution.testRunId = :testRunId
        and execution.sourceType =
          org.simplepoint.plugin.ai.skill.api.model.SkillExecutionSource.DRAFT
        and execution.debugMode =
          org.simplepoint.plugin.ai.skill.api.model.SkillDebugMode.MOCK
        and execution.deletedAt is null
      order by execution.testRunOrder asc
      """)
  ...
  @Query(
      value = """
          select execution.testRunId from AiSkillExecution execution
          where execution.skillId = :skillId
            and execution.scopeType = :scopeType
            and ((:tenantId is null and execution.tenantId is null)
              or execution.tenantId = :tenantId)
            and execution.testRunId is not null
            and execution.sourceType =
              org.simplepoint.plugin.ai.skill.api.model.SkillExecutionSource.DRAFT
            and execution.debugMode =
              org.simplepoint.plugin.ai.skill.api.model.SkillDebugMode.MOCK
            and execution.deletedAt is null
          group by execution.testRunId
          order by max(execution.createdAt) desc
          """,
      countQuery = """
          select count(distinct execution.testRunId)
          from AiSkillExecution execution
          where execution.skillId = :skillId
            and execution.scopeType = :scopeType
            and ((:tenantId is null and execution.tenantId is null)
              or execution.tenantId = :tenantId)
