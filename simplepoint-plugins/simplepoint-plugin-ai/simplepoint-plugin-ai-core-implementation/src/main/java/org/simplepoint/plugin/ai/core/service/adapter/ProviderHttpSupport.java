...
final class ProviderHttpSupport {
  ...
  static URI discoveryEndpoint(
      final String configuredUrl,
      final String baseUrl,
      final String defaultPath
  ) {
    if (configuredUrl == null || configuredUrl.isBlank()) {
      return endpoint(baseUrl, defaultPath);
    }
    return absoluteEndpoint(configuredUrl, "模型发现 URL");
  }

  static URI withQueryParameter(final URI uri, final String name, final String value) {
    if (value == null || value.isBlank()) {
      return uri;
    }
    String separator = uri.getRawQuery() == null || uri.getRawQuery().isBlank() ? "?" : "&";
    String encodedName = URLEncoder.encode(name, StandardCharsets.UTF_8);
    String encodedValue = URLEncoder.encode(value, StandardCharsets.UTF_8);
    return URI.create(uri + separator + encodedName + "=" + encodedValue);
  }

  private static URI absoluteEndpoint(final String value, final String label) {
    try {
