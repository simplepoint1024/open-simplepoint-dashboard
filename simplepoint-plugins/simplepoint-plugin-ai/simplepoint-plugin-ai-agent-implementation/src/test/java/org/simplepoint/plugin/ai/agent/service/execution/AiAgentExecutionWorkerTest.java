...
class AiAgentExecutionWorkerTest {

  @Test
  void boundsConcurrencyRenewsLeaseAndDrainsCleanly() throws Exception {
    final AiAgentExecutionCoordinator coordinator =
        mock(AiAgentExecutionCoordinator.class);
    final AiAgentExecutionRunner runner =
        mock(AiAgentExecutionRunner.class);
    ...
    ExecutionTask task = new ExecutionTask("execution-1", "worker-test", 7);
    ...
    doAnswer(invocation -> {
      ...
      return null;
    }).when(runner).execute(task);
    AiAgentExecutionWorker worker = new AiAgentExecutionWorker(
        coordinator,
        runner,
        properties,
        new AiPollingProperties(),
  ...
  private static void awaitIdle(
      final AiAgentExecutionWorker worker
