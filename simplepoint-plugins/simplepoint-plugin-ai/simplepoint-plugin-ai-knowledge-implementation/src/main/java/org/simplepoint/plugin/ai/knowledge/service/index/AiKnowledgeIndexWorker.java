...
@ConditionalOnProperty(
    prefix = AiKnowledgeProperties.PREFIX,
    name = "index-execution-enabled",
    havingValue = "true",
    matchIfMissing = true
)
public class AiKnowledgeIndexWorker {
  ...
  public AiKnowledgeIndexWorker(
    ...
    this.idleBackoff = new AiIdlePollBackoff(
        Duration.ofMillis(positive(properties.getIndexPollDelayMs(), 1000L)),
        pollingProperties
    );
  }
  ...
  public void poll() {
    if (!Boolean.TRUE.equals(properties.getIndexExecutionEnabled())
    ...
    idleBackoff.recordResult(!jobs.isEmpty());
