...
/**
 * Scope-owned Workflow identity with mutable display metadata.
 */
@Data
@Entity
@Table(
    name = "simpoint_ai_workflows",
    indexes = {
        @Index(
            name = "idx_simpoint_ai_workflow_scope",
            columnList = "scope_type, tenant_id"
        ),
        @Index(
            name = "idx_simpoint_ai_workflow_status",
            columnList = "status"
        )
    }
)
...
@NoArgsConstructor
@Schema(title = "AI Workflow")
public class AiWorkflowDefinition extends BaseEntityImpl<String> {
