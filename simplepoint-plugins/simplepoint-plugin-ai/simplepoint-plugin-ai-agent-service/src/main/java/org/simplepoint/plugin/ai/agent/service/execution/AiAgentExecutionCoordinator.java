...
public class AiAgentExecutionCoordinator {
  ...
  public boolean failModelAttempt(
    ...
    trace.setErrorCode(limit(errorCode, 64));
    trace.setErrorMessage(limit(errorMessage, 1024));
  ...
  /**
   * Stops before a pending Skill is submitted when control state changed.
   */
  @Transactional(rollbackFor = Exception.class)
  public boolean beforeSkill(final ExecutionTask task) {
    AiAgentExecution execution = requireOwned(task);
    if (stopAtCheckpoint(execution, Instant.now())) {
      executionRepository.save(execution);
      return false;
  ...
  /**
   * Persists a child Skill execution and releases the Agent lease.
   */
  @Transactional(rollbackFor = Exception.class)
  public boolean waitForSkill(
      final ExecutionTask task,
    ...
    trace.setSkillExecutionId(checkpoint.skillExecutionId());
    ...
    execution.setCurrentSkillExecutionId(checkpoint.skillExecutionId());
    ...
    publish(
        ...
        Map.of(
            ...
            "skillExecutionId", checkpoint.skillExecutionId()
        ),
        trace.getStartedAt()
    );
    return true;
  ...
  public boolean rejectSkillArguments(
    ...
    trace.setResponseHash(checkpoint.responseHash());
    trace.setOutputJson(checkpoint.outputJson());
    trace.setErrorCode(limit(checkpoint.errorCode(), 64));
    trace.setErrorMessage(limit(checkpoint.errorMessage(), 1024));
    trace.setStartedAt(Instant.now());
  ...
  private void failExecution(
    ...
    execution.setErrorCode(limit(errorCode, 64));
    execution.setErrorMessage(limit(errorMessage, 1024));
    execution.setPauseRequested(false);
  ...
  private static Map<String, Object> payloadError(
    ...
    return errorCode == null || errorCode.isBlank()
        ? Map.of() : Map.of("errorCode", limit(errorCode, 64));
  }
