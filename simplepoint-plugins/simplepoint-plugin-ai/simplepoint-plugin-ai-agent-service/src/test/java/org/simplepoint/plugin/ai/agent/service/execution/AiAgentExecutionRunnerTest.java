...
class AiAgentExecutionRunnerTest {
  ...
  @Test
  void completesPlainModelOutputWithStableObjectEnvelope() {
  ...
  void submitsValidPinnedSkillAndCheckpointsWait() {
    ...
    when(coordinator.beforeSkill(any())).thenReturn(true);
    when(skillExecutionService.startVersionForAgent(any()))
        .thenReturn(child);

    runner.execute(new ExecutionTask("execution-1", "worker-1", 1));

    verify(skillExecutionService).startVersionForAgent(any());
    verify(coordinator).waitForSkill(
        any(),
        org.mockito.ArgumentMatchers.argThat(checkpoint ->
            "skill-execution-1".equals(checkpoint.skillExecutionId())
