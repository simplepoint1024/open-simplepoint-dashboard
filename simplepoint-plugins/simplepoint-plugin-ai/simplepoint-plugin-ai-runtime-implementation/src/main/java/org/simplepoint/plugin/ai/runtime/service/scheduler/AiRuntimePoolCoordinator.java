...
public class AiRuntimePoolCoordinator {
  ...
  private static final int MAXIMUM_CONSECUTIVE_REPLICA_FAILURES = 3;

  private static final String REPLICA_FAILURE_CIRCUIT_PREFIX =
      "Runtime pool stopped after repeated replica failures: ";
  ...
  public int reconcile() {
    ...
    for (AiRuntimePool pool : pools) {
      ...
      List<AiRuntimeWorkload> active = all.stream()
          ...
          .toList();
      Optional<String> repeatedFailure = target > 0 && active.isEmpty()
          ? repeatedReplicaFailure(pool, all) : Optional.empty();
      if (target > 0
          && (replicaFailureCircuitOpen(pool)
            || repeatedFailure.isPresent())) {
        pool.setCurrentReplicas(0);
        pool.setReadyReplicas(0);
        pool.setLastReconciledAt(now);
        if (repeatedFailure.isPresent()) {
          pool.setStatus(RuntimePoolStatus.ERROR);
          pool.setLastError(
              REPLICA_FAILURE_CIRCUIT_PREFIX + repeatedFailure.orElseThrow()
          );
        }
        poolRepository.save(pool);
        continue;
  ...
  private AiRuntimeWorkload newReplica(
    ...
    workload.setRuntimeProfileId(pool.getRuntimeProfileId());
    workload.setRuntimeRevisionId(pool.getActiveRevisionId());
    workload.setRuntimeSpecJson(pool.getRuntimeSpecJson());
  ...
  private Optional<String> repeatedReplicaFailure(
      final AiRuntimePool pool,
      final List<AiRuntimeWorkload> workloads
  ) {
    List<AiRuntimeWorkload> latest = workloads.stream()
        .filter(workload -> workload.getCreatedAt() != null)
        .filter(workload -> pool.getLastActivityAt() == null
            || !workload.getCreatedAt().isBefore(pool.getLastActivityAt()))
        .sorted(Comparator.comparing(
            AiRuntimeWorkload::getReplicaSequence,
            Comparator.nullsLast(Comparator.reverseOrder())
        ))
        .limit(MAXIMUM_CONSECUTIVE_REPLICA_FAILURES)
        .toList();
    if (latest.size() < MAXIMUM_CONSECUTIVE_REPLICA_FAILURES
        || latest.stream().anyMatch(workload ->
            workload.getStatus() != RuntimeWorkloadStatus.FAILED)) {
      return Optional.empty();
    }
    String detail = latest.getFirst().getLastError();
    if (detail == null || detail.isBlank()) {
      detail = "OCI workload failed during startup";
    }
    detail = detail.trim();
    return Optional.of(detail.length() <= 768
        ? detail : detail.substring(0, 768));
  }

  private boolean replicaFailureCircuitOpen(final AiRuntimePool pool) {
    return pool.getStatus() == RuntimePoolStatus.ERROR
        && pool.getLastError() != null
        && pool.getLastError().startsWith(REPLICA_FAILURE_CIRCUIT_PREFIX);
  }
