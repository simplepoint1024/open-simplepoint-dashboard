...
class AccessCenterServiceImplTest {
  ...
  void saveRoleAuthorization_appliesOnlyPermissionDiffAndScope() {
    ...
    assertThat(authorizeCaptor.getValue().getDataScopeId()).isNull();
    assertThat(authorizeCaptor.getValue().getFieldScopeId()).isNull();
    verify(roleService).unauthorized("role1", Set.of("resources.old"));
    verify(roleService).updateScopeAssignment(org.mockito.ArgumentMatchers.argThat(scope ->
        "data1".equals(scope.getDataScopeId()) && "field1".equals(scope.getFieldScopeId())));
