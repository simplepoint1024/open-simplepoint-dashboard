    ...
    },
    {
      ...
      render: (_: unknown, node: RuntimeNode) => (
        <Space direction="vertical" size={0}>
          ...
          <Text type="secondary">{`Docker API ${node.engineApiVersion}`}</Text>
        </Space>
      ),
    },
    {
      ...
      render: (_: unknown, node: RuntimeNode) => (
        <Space wrap size={4}>
          <Tag color={node.seccompEnforced ? 'green' : 'red'}>seccomp</Tag>
          <Tag color={node.appArmorEnforced ? 'green' : 'default'}>AppArmor</Tag>
          <Tag color={node.requireSupplyChainAdmission ? 'green' : 'gold'}>Supply Chain</Tag>
          <Tag color={node.requireImageDigest ? 'green' : 'red'}>Digest</Tag>
        </Space>
      ),
    },
    {
        ...
        {detail && (
          <>
            <Descriptions bordered size="small" column={2}>
              <Descriptions.Item label="Node ID">{detail.nodeId}</Descriptions.Item>
              <Descriptions.Item label="Instance ID">{detail.instanceId}</Descriptions.Item>
              <Descriptions.Item label="Generation">{detail.generation}</Descriptions.Item>
              ...
              <Descriptions.Item label="seccomp">{detail.seccompProfileHash || '-'}</Descriptions.Item>
              <Descriptions.Item label="AppArmor">{detail.appArmorProfile || '-'}</Descriptions.Item>
