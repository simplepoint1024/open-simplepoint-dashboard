import api from '@/api';
import {Form as JsonSchemaForm} from '@rjsf/antd';
...
const requestedServerId = () => {
  ...
  return query ? new URLSearchParams(query).get('serverId') ?? undefined : undefined;
};

const Tools = () => {
  const config = api['ai-workbench.tools'];
  ...
  useEffect(() => {
    void get<Page<McpServer>>(config.serversUrl, {page: 0, size: 500})
      .then((page) => {
        const available = (page.content ?? []).filter((server) => (
          server.enabled !== false && server.status === 'READY'
        ));
        const preferred = requestedServerId();
  ...
  const loadCapabilities = useCallback(async () => {
    ...
    try {
      const [
        ...
        get<McpTool[]>(`${config.serversUrl}/${serverId}/tools`),
        get<McpResource[]>(`${config.serversUrl}/${serverId}/resources`),
        get<McpResourceTemplate[]>(`${config.serversUrl}/${serverId}/resource-templates`),
        get<McpPrompt[]>(`${config.serversUrl}/${serverId}/prompts`),
        get<Page<McpCapabilitySnapshotSummary>>(
          `${config.serversUrl}/${serverId}/snapshots`,
          {page: 0, size: 100},
        ),
      ]);
      setTools(nextTools);
    ...
    try {
      ...
      const [current, previous] = await Promise.all([
        get<McpCapabilitySnapshotDetails>(
          `${config.serversUrl}/${serverId}/snapshots/${summary.id}`,
        ),
        previousSummary
          ? get<McpCapabilitySnapshotDetails>(
            `${config.serversUrl}/${serverId}/snapshots/${previousSummary.id}`,
          )
  ...
  }, [config.serversUrl, serverId, snapshots, t]);
  ...
  const callTool = useCallback(async () => {
    ...
    setCalling(true);
    try {
      setResult(await post<ToolResult>(`${config.serversUrl}/${serverId}/tools/call`, {
  ...
  const readResource = useCallback(async () => {
    ...
    try {
      setCapabilityResult(await post(
        `${config.serversUrl}/${serverId}/resources/read`,
        {uri: readingResource},
      ));
    } catch (error) {
      message.error(resolveErrorMessage(
  ...
  }, [config.serversUrl, readingResource, serverId, t]);
  ...
  const getPrompt = useCallback(async () => {
    ...
    try {
      setCapabilityResult(await post(
        `${config.serversUrl}/${serverId}/prompts/get`,
  ...
  }, [argumentsJson, config.serversUrl, gettingPrompt, serverId, t]);
                ...
                <Table
                  ...
                  columns={[
                    ...
                    },
                    {
                      ...
                      render: (_value: unknown, snapshot: McpCapabilitySnapshotSummary) => (
                        <Space wrap>
                          <Tag>{`T ${snapshot.toolCount}`}</Tag>
                          <Tag>{`R ${snapshot.resourceCount}`}</Tag>
                          <Tag>{`RT ${snapshot.resourceTemplateCount}`}</Tag>
                          <Tag>{`P ${snapshot.promptCount}`}</Tag>
                        </Space>
                      ),
                    },
                    {
          ...
          {result && (
            <Form.Item label={t('ai.mcp.tools.result', '调用结果')}>
              <Tag color={result.error ? 'red' : 'green'}>
                {result.error ? 'MCP ERROR' : 'SUCCESS'}
              </Tag>
              <Paragraph>
        ...
        {snapshotComparison && (
          <>
            ...
            <Table
              ...
              pagination={false}
              columns={[
                {title: t('ai.mcp.snapshots.capability', '能力类型'), dataIndex: 'type', width: 140},
                {
