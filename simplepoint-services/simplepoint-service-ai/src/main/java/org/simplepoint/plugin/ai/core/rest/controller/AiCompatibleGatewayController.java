...
import static org.simplepoint.plugin.ai.core.rest.gateway.AiCompatibleGatewayErrors.AUTHENTICATION_MESSAGE;
import static org.simplepoint.plugin.ai.core.rest.gateway.AiCompatibleGatewayErrors.GENERATION_FAILURE_CODE;
import static org.simplepoint.plugin.ai.core.rest.gateway.AiCompatibleGatewayErrors.GENERATION_FAILURE_MESSAGE;
import static org.simplepoint.plugin.ai.core.rest.gateway.AiCompatibleGatewayErrors.INTERNAL_FAILURE_MESSAGE;
import static org.simplepoint.plugin.ai.core.rest.gateway.AiCompatibleGatewayErrors.INVALID_REQUEST_MESSAGE;
import static org.simplepoint.plugin.ai.core.rest.gateway.AiCompatibleGatewayErrors.PERMISSION_MESSAGE;
import static org.simplepoint.plugin.ai.core.rest.gateway.AiCompatibleGatewayErrors.RATE_LIMIT_MESSAGE;
import static org.simplepoint.plugin.ai.core.rest.gateway.AiCompatibleGatewayErrors.SERVICE_BUSY_MESSAGE;
import static org.simplepoint.plugin.ai.core.rest.gateway.AiCompatibleGatewayErrors.UPSTREAM_FAILURE_MESSAGE;
...
public class AiCompatibleGatewayController {
  ...
  private static final MediaType EVENT_STREAM_UTF8 = new MediaType(
  ...
  private ResponseEntity<SseEmitter> startOpenAiStream(
    ...
    return ResponseEntity.ok().contentType(EVENT_STREAM_UTF8).body(emitter);
  }
  ...
  private void consumeOpenAi(
    ...
    try {
      ...
      log.warn("OpenAI chat completion stream failed", ex);
      if (!state.failed) {
        sendOpenAiStreamFailure(emitter);
      }
      emitter.complete();
    }
  }
  ...
  private void sendOpenAiEvent(
    ...
    if (event.type() == EventType.ERROR) {
      state.failed = true;
      logStreamFailure("OpenAI chat completions", event);
      sendData(emitter, compatibilityMapper.openAiError(
          GENERATION_FAILURE_MESSAGE, "api_error", GENERATION_FAILURE_CODE));
      return;
    }
  ...
  private void consumeAnthropic(
    ...
    try {
      ...
      log.warn("Anthropic messages stream failed", ex);
      if (!state.failed) {
        sendAnthropicStreamFailure(emitter);
      }
      emitter.complete();
    }
  }
  ...
  private void consumeResponses(
    ...
    try {
      stream.consume(event -> {
        if (event.type() == EventType.ERROR) {
          failed.set(true);
          logStreamFailure("OpenAI responses", event);
        }
        responsesProtocol.streamEvents(state, event)
      ...
      log.warn("OpenAI responses stream failed", ex);
      if (!failed.get()) {
        responsesProtocol.streamFailureEvent(state)
            .forEach(responseEvent -> sendResponseEvent(emitter, responseEvent));
      }
      emitter.complete();
    }
  ...
  private void sendAnthropicEvent(
    ...
    switch (event.type()) {
      ...
      case ERROR -> {
        state.failed = true;
        logStreamFailure("Anthropic messages", event);
        sendAnthropicStreamFailure(emitter);
      }
      default -> {
  ...
  private ResponseEntity<String> openAiError(final RuntimeException ex) {
    log.warn("OpenAI compatible gateway request failed", ex);
  ...
  private ResponseEntity<String> anthropicError(final RuntimeException ex) {
    log.warn("Anthropic compatible gateway request failed", ex);
    ErrorStatus error = status(ex);
    return ResponseEntity.status(error.status)
        .contentType(MediaType.APPLICATION_JSON)
  ...
  private static ErrorStatus status(final RuntimeException ex) {
    if (ex instanceof AiGatewayAccessException access) {
      return switch (access.getFailureType()) {
        case AUTHENTICATION -> new ErrorStatus(401, AUTHENTICATION_MESSAGE,
            "authentication_error", "invalid_api_key", "authentication_error");
        case PERMISSION -> new ErrorStatus(403, PERMISSION_MESSAGE,
            "permission_error", "permission_denied", "permission_error");
        case RATE_LIMIT -> new ErrorStatus(429, RATE_LIMIT_MESSAGE,
            "rate_limit_error", "rate_limit_exceeded", "rate_limit_error");
      };
    }
    if (ex instanceof IllegalArgumentException) {
      return new ErrorStatus(400, INVALID_REQUEST_MESSAGE,
          "invalid_request_error", "invalid_request", "invalid_request_error");
    }
    if (ex instanceof AiProviderRequestException) {
      return new ErrorStatus(502, UPSTREAM_FAILURE_MESSAGE,
          "api_error", "upstream_error", "api_error");
    }
    if (ex instanceof RejectedExecutionException) {
      return new ErrorStatus(503, SERVICE_BUSY_MESSAGE,
          "api_error", "service_unavailable", "overloaded_error");
    }
    return new ErrorStatus(500, INTERNAL_FAILURE_MESSAGE,
  ...
  private void sendOpenAiStreamFailure(final SseEmitter emitter) {
    sendData(emitter, compatibilityMapper.openAiError(
        GENERATION_FAILURE_MESSAGE, "api_error", GENERATION_FAILURE_CODE));
  }

  private void sendAnthropicStreamFailure(final SseEmitter emitter) {
    sendEvent(emitter, "error", compatibilityMapper.anthropicError(
        GENERATION_FAILURE_MESSAGE, "api_error"));
  }

  private static void logStreamFailure(
      final String protocol,
      final GenerationEvent event
  ) {
    log.warn(
        "{} stream reported generation failure: code={}, detail={}",
        protocol,
        event.errorCode(),
        event.errorMessage()
    );
